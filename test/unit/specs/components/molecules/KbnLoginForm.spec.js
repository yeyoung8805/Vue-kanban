...
describe('KbnLoginForm', () => {
    describe('프로퍼티', () => {
            ...
            describe('email', () => {
                ...
                describe('format', () => {
                    describe('이메일 주소 형식이 아닌 값', () => {
                        it('validation.email.format 이 invalid 임', () => {
                            loginForm.setData({
                                email: 'foobar'
                            })
                    ...
                    describe('이메일 주소 형식인 값', () => {
                        it('validation.email.required이 valid 임', () => {
                            loginForm.setData({
                                email: 'foo@domain.com'
                            })
        ...
        describe('disableLoginAction', () => {
            ...
            describe('유효성 검사 모든 항목 OK이고 로그인처리 중이 아님', () => {
                it('유효한 로그인 처리', () => {
                    loginForm.setData({
                        email: 'foo@domain.com',
                        password: '12345678'
        ...
        describe('onLogin', () => {
            ...
            describe('resolve', () => {
                it('resolve 됨', done  =>  {
                    ...
                    expect(onLoginStub.called).to.equal(false) //아직 resolve 되지 않음
                    ...
                    loginForm.vm.$nextTick(() => {
                        ...
                        expect(authInfo.email).to.equal(loginForm.email)
                        expect(authInfo.password).to.equal(loginForm.vm.password)
                        loginForm.vm.$nextTick(() => { //resolve에서 상태 반영
            ...
            describe('reject', () => {
                it('reject 됨', done => {
                    ...
                    //퀵 이벤트
                    loginForm.find('button').trigger('click')
                    expect(onLoginStub.called).to.equal(false) //아직reject 되지 않음
                    expect(loginForm.vm.error).to.equal('') //오류 메시지 초기화
                    expect(loginForm.vm.disableLoginAction).to.equal(true) // 로그인 액션 불가
